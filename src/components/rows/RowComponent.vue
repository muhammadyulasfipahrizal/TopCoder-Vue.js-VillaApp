<template>
  <div class="container">
    <div class="list-input">
      ...
      <SearchInput @update-search-term="updateSearchTerm" class="search-input" />
    </div>
    <ListProperty />
    <RowUi
      v-for="villa in filteredVillas"
      :key="villa.id"
  ...
  computed: {
    ...
    filteredVillas() {
      return this.villas.filter(villa => {
        ...
        return villa.name.toLowerCase().includes(searchTerm) ||
          villa.location.toLowerCase().includes(searchTerm) ||
          villa.amenities.toLowerCase().includes(searchTerm);
      });
    },
  },
  watch: {
